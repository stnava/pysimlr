...
Absolute Canonical Covariance (ACC) encourages shared alignment. Derived from the Orthogonal Procrustes problem [@schonemann1966generalized] $\min_{R^T R = I} \| Z_m - Z_l R \|_F^2$:
...
```
LEND Architecture. The "First-Layer Contract" restricts the encoder to a single linear projection $V$. This ensures that every latent factor can be exactly decomposed into the original features, while the Deep MLP decoder ($g_\theta$) [@goodfellow2016deep] handles the non-linear reconstruction of the data manifold.
:::
...
The most advanced variant, explicitly partitioning the latent space to sequester view-specific noise via the **Shared-Private Disentanglement Mechanism** [@bousmalis2016domain; @salzmann2010factorized].
