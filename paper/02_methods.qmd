...
The fundamental SiMLR objective is to minimize a joint loss function $\mathcal{L}$ that balances intra-modality reconstruction, inter-modality alignment, and representation stability via discrete penalty weights (e.g., $\lambda_{\text{sim}}, \lambda_{\text{var}}, \lambda_{\text{cov}}$). To prevent trivial "mode collapse" (where representations collapse to a constant point to perfectly satisfy alignment), we decouple the target generation from the parameter update via a formal **Alternating Optimization** framework.

At optimization step $t$, we first compute an auxiliary consensus target $U_{\neq m}^{(t)}$ for each modality $m$. Under our default **Leave-One-Out (LOO) topology**, this target is constructed exclusively from the latent scores of all *other* modalities from the previous step:
$$ U_{\neq m}^{(t)} = \text{Consensus}(\{Z_i^{(t-1)}\}_{i \neq m}) $$

The model parameters $\{f_m, g_m\}$ are then updated by minimizing the loss with $U_{\neq m}^{(t)}$ held constant:

$$
\mathcal{L}^{(t)} = \sum_{m=1}^M \mathcal{L}_{\text{recon}}(X_m, \hat{X}_m) + \lambda_{\text{sim}} \sum_{m=1}^M \mathcal{L}_{\text{align}}(Z_m, U_{\neq m}^{(t)}) + \lambda_{\text{var}} \mathcal{L}_{\text{var}} + \lambda_{\text{cov}} \mathcal{L}_{\text{cov}} + \gamma \sum_{m=1}^M \Omega(V_m)
$$
...
*   $\mathcal{L}_{\text{align}}(Z_m, U_{\neq m}^{(t)})$ measures the cross-modal alignment of each latent space $Z_m$ to its fixed target consensus $U_{\neq m}^{(t)}$. By evaluating the alignment against a fixed target generated in the previous step, we ensure gradients only flow through the modality-specific encoder. This forces the encoder to discover features that match the consensus rather than allowing the consensus to simultaneously drift to match the features. The strict cross-modal prediction pathway of the LOO topology acts as a powerful structural regularizer, preventing high-capacity deep networks from "cheating" by memorizing their own idiosyncratic noise. A computationally simpler "star-topology" (where all modalities align to a single shared $U^{(t)}$) is also supported for faster approximation with $O(M)$ complexity.
...
$$ \mathcal{L}_{ACC} = - \frac{1}{M} \sum_{m=1}^M \text{Tr}(Z_m^T U_{\neq m}^{(t)}) $$
...
#### Spectral Triage and Modality Contribution Index (MCI)

To ensure the stability of the consensus representation, we introduce **Spectral Triage**, a self-healing mechanism that automatically identifies and quarantines high-dimensional noise traps ("Rogue modalities"). This is operationalized through the **Modality Contribution Index (MCI)**, which weights each modality's contribution to the shared consensus $U$ based on its alignment quality and spectral structure.

The MCI supports four distinct metrics for quantifying modality contribution:

1. **Procrustes $R^2$:** Measures the variance explained between the modality latent $Z_m$ and the current consensus $U$ after orthogonal alignment.
2. **RV-Coefficient:** A multivariate generalization of the squared Pearson correlation coefficient, measuring the similarity between the covariance structures of $Z_m$ and $U$.
3. **CCA:** Uses the leading canonical correlation between $Z_m$ and $U$ to assess alignment.
4. **Procrustes $R^2$ with Spectral Sharpness (Default):** Our preferred metric that augments alignment with a penalty for spectral isotropic noise.

##### Spectral Sharpness

Spectral Sharpness $S$ is defined as the ratio of the leading singular value to the sum of all singular values in the modality's latent projection:
$$ S = \frac{\sigma_1}{\sum_{i=1}^k \sigma_i} $$
where $\sigma_i$ are the singular values of $Z_m$. This metric distinguishes structured biological signal (characterized by a "sharp" spectrum with dominant components) from isotropic accidental correlation (characterized by a "flat" spectrum where noise is distributed evenly across dimensions).

##### Sigmoid Gating

The MCI weights $\omega_m$ are computed using a **Sigmoid Gating** mechanism that maps the chosen metric $M_m$ to a contribution weight:
$$ \omega_m = \frac{1}{1 + e^{-\beta(M_m - \tau)}} $$
where $\tau$ is a significance threshold and $\beta$ is a steepness parameter. This ensures that modalities failing to meet the "Spectral Triage" criteria are automatically quarantined (weighted near zero), preventing them from polluting the shared consensus. The final weighted consensus is normalized to maintain representation stability.
...
graph LR
    ...
    Other[Other Latents] --> MCI[MCI Weighting]
    MCI --> U["Target U_{≠m}^{(t)}"]
    Z -.->|Alignment Loss| U
    ...
    style MCI fill:#fff3e0,stroke:#e65100,color:#000
```
:::
...
NEDPP Architecture. The latent space is partitioned into a shared consensus $U_{\neq m}^{(t)}$ (aligned across views) and a private space $P$ (view-specific). A cross-covariance penalty ensures that $P$ captures only idiosyncratic variance, preventing it from polluting the shared signal.
:::
