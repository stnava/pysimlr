...
def test_linear_parity_contract():
    ...
    assert (lend_rec - simlr_rec) > -0.05, f"Recovery gap too large negative: {(simlr_rec - lend_rec):.4f}"
    ...
    assert (lend_r2 - simlr_r2) > -0.05, f"R2 gap too large negative: {(simlr_r2 - lend_r2):.4f}"
