...
def run_comprehensive_search(file_path):
    print("=== PPMI+ADNI Comprehensive Model & Consensus Search ===")
    ...
    energy_options = ['logcosh', 'nc']
    mixing_options = ['newton', 'svd']
    ...
    for m_name, m_fn in model_fns.items():
        for mai in mai_options:
            for energy in energy_options:
                for mix in mixing_options:
                    print(f"\nTesting: {m_name} | MAI: {mai} | Energy: {energy} | Mix: {mix}")
                    try:
                        start_t = time.time()
                        
                        # Fit model on Train (fewer epochs for search speed)
                        res = m_fn(
                            Xs_train, k=8, epochs=60, 
                            hidden_dims=[64, 32],
                            dynamic_weights=mai,
                            energy_type=energy, 
    ...
    print("\n" + "="*80)
    print("COMPREHENSIVE SEARCH RESULTS (Test Set Performance)")
    print("="*80)
