...
def run_multimodal_search(file_path):
    print("=== PPMI+ADNI Architecture & MAI Search ===")
    ...
    df = df[df['AsynStatus'] != 'PosMSA'].copy()
    df = df.dropna(subset=['AsynStatus']).copy()
    ...
    for arch in architectures:
        for mai in mai_options:
            print(f"\nTesting Arch: {arch}, MAI: {mai}")
            start_t = time.time()
            
            # Fit SiMLR on Train
            res = lend_simr(
                Xs_train, k=10, epochs=80, 
                hidden_dims=arch,
                dynamic_weights=mai,
                energy_type='logcosh', 
    ...
    print("\n" + "="*50)
    print("SEARCH RESULTS SUMMARY (Test Set Performance)")
    print("="*50)
