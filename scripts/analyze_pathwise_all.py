...
def pathwise_permutation_test(data, k, topology="graph", path_graph=None, n_perms=100, **kwargs):
    # 1. Observed Fit
...
def run_comprehensive_pathwise_analysis():
    ...
    for name, config in models.items():
        ...
        stats = pathwise_permutation_test(
            data, k=3, topology=config["topology"], path_graph=config["path_graph"], 
            n_perms=100, epochs=100, energy_type="acc", mixing_algorithm="svd", 
            sparseness_quantile=0.5, sim_weight=2.0, verbose=False
        )
        results[name] = stats
        
    print("\n" + "="*85)
    print("COMPREHENSIVE PATHWISE SIGNIFICANCE REPORT (LEND ARCHITECTURE)")
    print("="*85)
...
if __name__ == "__main__":
    run_comprehensive_pathwise_analysis()
