...
def run_pooled_moca_non_adjusted_mapped(file_path):
    print("=== PPMI+ADNI MOCA Study (Non-Adjusted Counterparts) ===")
    ...
    def get_non_adjusted_counterparts(prefix):
        # 1. Find the adjusted columns first
        ...
        # 2. Map to non-adjusted counterparts
        # If the format is 'variable_adjusted', we want 'variable'
        # We need to verify these exist
        mapped_cols = []
        for c in adj_cols:
            raw_c = c.replace('_adjusted', '').replace('.adjusted', '') # handle both common delimiters
            if raw_c in df.columns:
                mapped_cols.append(raw_c)
            else:
                # Fallback: if not found exactly, skip this pair to keep it clean
                pass
        return mapped_cols

    t1_cols = get_non_adjusted_counterparts('T1Hier')
    dti_cols = get_non_adjusted_counterparts('DTI_mean_fa')
    rsf_cols = [c for c in get_non_adjusted_counterparts('rsfMRI_fcnxpro129_') if '_2_' in c]

    print(f"Features Identified (Mapped from Adjusted counterparts):")
    ...
    best_idx = np.argmax(np.abs([pearsonr(U[:, i], y)[0] for i in range(U.shape[1])]))
    
    output_dir = 'clinical_results_raw'
    os.makedirs(output_dir, exist_ok=True)
    sns.set_theme(style="whitegrid")

    plt.figure(figsize=(8, 6))
    sns.regplot(x=U[:, best_idx], y=y, scatter_kws={'alpha':0.3, 'color':'#2c3e50'}, line_kws={'color':'#e74c3c'})
    plt.title(f"Non-Adjusted Analysis (Mapped Features): Imaging vs. MOCA\nCorrelation: {pearsonr(U[:, best_idx], y)[0]:.3f}", fontsize=14)
    plt.xlabel(f"Shared Imaging Latent (Dim {best_idx})", fontsize=12)
    plt.ylabel("Cognitive Score (MOCA)", fontsize=12)
    plt.savefig(f'{output_dir}/moca_vs_latent_raw.png', dpi=300)
    ...
    for i, (name, v_mat) in enumerate(zip(view_names, V_mats)):
        ...
        sns.barplot(x=v_weights[top_idx], y=top_labels, hue=top_labels, palette='rocket', legend=False)
        plt.title(f"Top {name} RAW Features Driving Cognition", fontsize=14)
        plt.tight_layout()
        plt.savefig(f'{output_dir}/top_features_{name}_raw.png', dpi=300)

    print(f"\nAnalysis complete. Visuals in '{output_dir}/'.")

if __name__ == "__main__":
    path = "../multidisorder/data/ppmiadni_filtered.csv"
    run_pooled_moca_non_adjusted_mapped(path)
