...
def preprocess_view(df_sub, return_cols=False):
    ...
    1. Filter out forbidden strings.
    2. Enforce 'adjusted' if requested.
    3. CLIP extreme values (10 sigma) to prevent latent outliers.
    """
    ...
    # OUTLIER FIX: Clip extreme values at +/- 10 standard deviations
    # This prevents artifacts (like the 38-sigma DTI spike) from ruining the latent space
    X_clipped = np.clip(X_sc, -10, 10)
    
    # Re-standardize slightly after clipping to maintain unit variance
...
def run_pooled_moca_refined(file_path):
    print("=== PPMI+ADNI Refined MOCA Study: Fixed Outliers & Filtered Features ===")
    ...
    def get_final_cols(prefix, require_adjusted=True):
    ...
    # Define Imaging Views with strict 'adjusted' requirement
    t1_cols_raw = get_final_cols('T1Hier')
    dti_cols_raw = get_final_cols('DTI_mean_fa')
    rsf_cols_raw = [c for c in get_final_cols('rsfMRI_fcnxpro129_') if '_2_' in c]

    print(f"Features Identified (Adjusted & Non-Forbidden):")
    ...
    # 1. Prediction Benchmark (75/25) - Using the previous winner: LEND + MAI
    ...
    print(f"\nRefined Results (Test Set):")
    print(f"  R^2: {r2:.4f}")
    print(f"  Correlation: {corr:.4f} (p={pval:.4e})")

    # 2. Check for remaining outliers in latents
    U_all = np.concatenate([U_train, U_test], axis=0)
    max_z = np.max(np.abs((U_all - np.mean(U_all, axis=0)) / np.std(U_all, axis=0)))
    print(f"  Max latent Z-score (post-fix): {max_z:.2f} (Previously >22)")

    # 3. Final Re-fit and Visualization
    print("\nGenerating final clinician interpretation plots...")
    res_viz = lend_simr(Xs, k=12, epochs=150, hidden_dims=[64, 32], dynamic_weights=True, energy_type='regression', mixing_algorithm='newton', verbose=False)
    ...
    for i, (name, v_mat) in enumerate(zip(view_names, V_mats)):
        ...
        sns.barplot(x=v_weights[top_idx], y=top_labels, palette='coolwarm')
        plt.title(f"Top {name} Drivers of Cognitive Signal (Refined)", fontsize=14)
        plt.tight_layout()
        plt.savefig(f'{output_dir}/top_features_{name}_refined.png', dpi=300)

    print(f"\nRefined analysis complete. Visuals in '{output_dir}/'.")

if __name__ == "__main__":
    path = "../multidisorder/data/ppmiadni_filtered.csv"
    run_pooled_moca_refined(path)
