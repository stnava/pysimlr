...
def run_systematic_perm_tests(n_perms=500, lambda1=0.25, alpha=0.8, lambda2=1.0, sparsity_thresh=1e-3):
    ...
    # ==============================================================================
    # STEP 1: EXTRACT LATENTS BY RUNNING SIMLR-FLOW-V ON RAW MODALITIES
    # ==============================================================================
    print("=================================================================")
    print("STEP 1: EXTRACT LATENTS BY RUNNING SIMLR-FLOW-V ON RAW MODALITIES")
    print("=================================================================")
    t0_flow = time.time()
    ...
    print(f"Running Simlr-Flow-V on device: {device}...")
    ...
    # Stack latents horizontally to form 128 predictors (32 components x 4 modalities)
    ...
    # Construct covariates design matrix matching R (including column of ones for intercept)
    ...
    # ==============================================================================
    print("\n\n" + "="*80)
    print("                RAW FLOW SYSTEMATIC SIGNIFICANCE SUMMARY REPORT")
    print("="*80)
    ...
    df_summary.to_csv("expart_raw_flow_significance_results.csv", index=False)
    print("Saved systematic results to: expart_raw_flow_significance_results.csv")
    
    # Save a markdown report version
    artifact_path = "/Users/stnava/.gemini/antigravity-cli/brain/fda666c5-3968-404e-b3e4-f71672770deb/raw_flow_significance_report.md"
    with open(artifact_path, "w") as f:
        f.write("# Raw Modality Flow-Simlr-V Systematic Significance Report\n\n")
        f.write(f"Analyzed on the full **EXPART cohort ($N=98$)** using **{n_perms} target-shuffling permutations** based on joint latents learned directly from the **4 raw brain modality predictors** (T1, DTI, rsfMRI, Perfusion) using **Simlr-Flow-V ($k=32$ per modality)**.\n\n")
        f.write("## Parameters\n")
        f.write("### Simlr-Flow-V Configuration\n")
        f.write("- Number of latents per view ($k$): 32 (128 features total)\n")
        ...
        f.write("1. **Simlr-Flow-V learned directly from Raw Features:** Extracting 128 latent dimensions from the full high-dimensional raw voxel-wise/region-wise modality features preserves non-linear joint couplings that PCA might discard.\n")
