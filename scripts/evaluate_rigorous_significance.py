...
from pysimlr import simlr, flow_simr_v
from pysimlr.utils import set_all_seeds, adjusted_rvcoef
from scipy.stats import pearsonr

def run_rigorous_significance(n_perms=200):
    set_all_seeds(42)
    print("=================================================================")
    ...
    # -----------------------------------------------------------------
    # Freedman-Lane Residualization logic
    # -----------------------------------------------------------------
    ...
    # Standardize observed residuals for linear SiMLR
    ...
    print("\n=================================================================")
    print("STEP 2: FIT OBSERVED LINEAR SIMLR MODEL ON RESIDUALS")
    print("=================================================================")
    
    k = 2
    res_obs = simlr(
        [scaled_bl_obs, scaled_mh_obs, scaled_mr_obs, scaled_pe_obs],
        k=k,
        energy_type='regression',
        verbose=False
    )
    ...
    # Projections
    proj_bl_obs = scaled_bl_obs @ res_obs['v'][0].numpy()
    proj_mh_obs = scaled_mh_obs @ res_obs['v'][1].numpy()
    proj_mr_obs = scaled_mr_obs @ res_obs['v'][2].numpy()
    proj_pe_obs = scaled_pe_obs @ res_obs['v'][3].numpy()
    
    obs_coupling = adjusted_rvcoef(torch.as_tensor(proj_bl_obs).float(), torch.as_tensor(proj_mr_obs).float())
    print(f"Observed linear Blast <-> MRI coupling (Adjusted RV): {obs_coupling:.4f}")
    
    # Observed correlation for Latent Dim 1 of Blast with CVLTLongDelay
    obs_lat_bl_dim1 = proj_bl_obs[:, 0]
    target_pe_idx = npsy_pe_cols.index('npsy_CVLTLongDelayFreeRecall_Raw')
    obs_corr, obs_p = pearsonr(obs_lat_bl_dim1, pe_res[:, target_pe_idx])
    print(f"Observed linear Blast Latent Dim 1 <-> CVLTLongDelay correlation: r={obs_corr:.4f} (p={obs_p:.4f})")
    ...
    for _ in range(n_perms):
        # Shuffling the already-projected latent coordinates
        ...
        obs_lat_bl_perm = rng.permutation(obs_lat_bl_dim1)
        posthoc_corr_nulls.append(pearsonr(obs_lat_bl_perm, pe_res[:, target_pe_idx])[0])
    ...
    # -----------------------------------------------------------------
    # Rigorous Refit-under-Permutation significance (Proposed design)
    # -----------------------------------------------------------------
    print(f"\n--- Running Rigorous Refit-under-Permutation Tests ({n_perms} perms, refitting simlr each time) ---")
    refit_rv_nulls = []
    refit_corr_nulls = []
    
    # Pre-calculated covariate design matrix for projection
    X_cov = np.hstack([np.ones((covariates.shape[0], 1)), covariates])
    
    for b in range(n_perms):
        if (b+1) % 20 == 0:
            print(f"  Completed {b+1}/{n_perms} permutations...")
            
        # 1. Permute the residuals of the target view (Blast)
        bl_res_perm = rng.permutation(bl_res, axis=0)
        
        # 2. Reconstruct under the null: add back the covariate predictions
        bl_null = bl_pred + bl_res_perm
        
        # 3. Re-residualize to get the Freedman-Lane matrices
        # For Blast (re-project to clean up any slight numerical leakage)
        beta_bl_null = np.linalg.pinv(X_cov.T @ X_cov) @ X_cov.T @ bl_null
        bl_adj_null = bl_null - X_cov @ beta_bl_null
        
        # 4. Standardize all views for the null fit
        ...
        # 5. Refit the linear SiMLR model
        res_null = simlr(
            [scaled_bl_null, scaled_mh, scaled_mr, scaled_pe],
            k=k,
            energy_type='regression',
            verbose=False
        )
        
        # 6. Extract the projections of the refitted model
        proj_bl_null = scaled_bl_null @ res_null['v'][0].numpy()
        proj_mr_null = scaled_mr @ res_null['v'][2].numpy()
        
        # 7. Compute the null statistics
        ...
        # Latent Dim 1 correlation with CVLTLongDelay
        lat_bl_dim1_null = proj_bl_null[:, 0]
        r_null, _ = pearsonr(lat_bl_dim1_null, pe_res[:, target_pe_idx])
    ...
    ax = axes[0]
    ax.hist(posthoc_rv_nulls, bins=20, alpha=0.5, label="Post-hoc Null (No Refit)", color='blue', edgecolor='k')
    ax.hist(refit_rv_nulls, bins=20, alpha=0.5, label="Rigorous Null (Refit)", color='green', edgecolor='k')
    ax.axvline(obs_coupling, color='red', linestyle='--', lw=2, label=f"Observed RV = {obs_coupling:.4f}")
    ax.set_title(f"Modality Coupling Null Distributions\nPost-hoc p = {posthoc_rv_p:.3f} | Refit p = {refit_rv_p:.3f}", fontsize=11, fontweight='bold')
    ...
    ax = axes[1]
    ax.hist(posthoc_corr_nulls, bins=20, alpha=0.5, label="Post-hoc Null (No Refit)", color='blue', edgecolor='k')
    ax.hist(refit_corr_nulls, bins=20, alpha=0.5, label="Rigorous Null (Refit)", color='green', edgecolor='k')
    ax.axvline(obs_corr, color='red', linestyle='--', lw=2, label=f"Observed r = {obs_corr:.4f}")
    ax.set_title(f"Latent Dim 1 Correlation Null Distributions\nPost-hoc p = {posthoc_corr_p:.3f} | Refit p = {refit_corr_p:.3f}", fontsize=11, fontweight='bold')
    ...
    plt.savefig("expart_report_figures/significance_comparison.png", bbox_inches='tight', dpi=300)
    plt.close()
    print("\nSaved comparison figure: expart_report_figures/significance_comparison.png")
    
    # Save results to a CSV file for report integration
    ...
    summary_df.to_csv("expart_rigorous_significance_results.csv", index=False)
    print("Saved comparison summary to: expart_rigorous_significance_results.csv")
    print(summary_df.to_string(index=False))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--perms", type=int, default=200, help="Number of permutations to run")
    args = parser.parse_args()
    
    run_rigorous_significance(n_perms=args.perms)
